...
import { fetchRegistryStatus, type RegistryStatusResponse } from "../api/registry";

export function useRegistryStatus(enabled: boolean) {
  ...
  const refresh = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        setStatus(await fetchRegistryStatus(signal));
        setError(null);
      } catch (requestError) {
