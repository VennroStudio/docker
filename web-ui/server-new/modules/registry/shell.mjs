...
export function streamRegistryShell(req, res, container) {
  const [command, args] = registryShellCommand(container);
  ...
  req.on("close", () => stopRegistryShell(sessionId));
  writePromptCommand(child);
}

export function isRegistryShellSession(sessionId) {
  return typeof sessionId === "string" && sessionId.startsWith(sessionPrefix) && sessions.has(sessionId);
}

export function writeRegistryShellInput(sessionId, input) {
...
export function stopRegistryShell(sessionId) {
