...
export function streamRedisShell(req, res, container) {
  const [command, args] = redisShellCommand(container);
  ...
  req.on("close", () => stopRedisShell(sessionId));
  writePromptCommand(child);
}

export function isRedisShellSession(sessionId) {
  return typeof sessionId === "string" && sessionId.startsWith(sessionPrefix) && sessions.has(sessionId);
}

export function writeRedisShellInput(sessionId, input) {
...
export function stopRedisShell(sessionId) {
