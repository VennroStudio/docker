...
export function createShellSessionManager({ env = () => process.env, prefix, resolveCommand }) {
  const sessions = new Map();
  const sessionPrefix = `${prefix}:`;

  async function stream(req, res, ...params) {
    const [command, args] = await resolveCommand(...params);
    ...
    req.on("close", () => stop(sessionId));
    writePromptCommand(child);
  }

  function is(sessionId) {
  ...
  function write(sessionId, input) {
    ...
    if (!child || child.killed || !child.stdin.writable) throw new Error("Shell session is not running");
    child.stdin.write(
      `printf '${startMarker}\\n'\n${input.trimEnd()}\n__infra_status=$?\nprintf '${endMarker}=%s\\n' "$__infra_status"\n`,
    );
    writePromptCommand(child);
