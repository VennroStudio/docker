...
Дополнительно на странице есть кнопка генерации `.env` из сохраненного `settings.json`. При несохраненных изменениях кнопку нужно сначала разблокировать сохранением настроек.
...
- Settings UI не синхронизирует `.env` автоматически; `.env` обновляется отдельной кнопкой генерации.
